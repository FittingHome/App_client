...
const FittingCart = (props) => {
  ...
  useEffect(() => {
    const getGarmentsIds = () => {
      var ids = [];

      if (garments.top.filename !== "") {
        ids.push(garments.top.filename);
      }
      if (garments.down.filename !== "") {
        ids.push(garments.down.filename);
      }
      console.log("ids", ids);
      return ids;
    };
    const getFilename = async () => {
      ...
      const garmentsIds = getGarmentsIds();
      console.log("garmentIds", garmentsIds);
    ...
    console.log(getGarmentsIds().length);
    if (getGarmentsIds().length > 0) {
      getFilename();
    }
  ...
  const getGarmentFilename = (product, color, size) => {
  ...
  function changeSize(btn, product) {
    ...
    newGarment["color"] =
      newGarment["id"] === btn.id ? newGarment["color"] : "";
        ...
        <div className="col-md-12 mb-3">
          <div className="card h-100 text-center p-4" key={product._id}>
            <img
              ...
              src={`http://91.172.40.53:8080/image?id=${product.imagePath.substring(
                0,
                36
              )}`}
            ...
            <div className="card-body">
              ...
              <div className="buttons justify-content-center d-flex my-4">
                {sizes.map((size) => {
                  const isActive =
                    garments[product.type]["id"] === product._id &&
                    garments[product.type]["size"] === size
                      ? "active"
                      : "";
                  return (
                    <button
                      ...
                      value={size}
                      onClick={(e) => changeSize(e.target, product)}
                    >
                      {size}
                    </button>
                  );
              ...
              <div className="buttons justify-content-center d-flex colors my-4">
                {colors.map((color) => {
                  const isActive =
                    garments[product.type]["id"] === product._id &&
                    garments[product.type]["color"] === color
                      ? "active"
                      : "";
                  return (
                    <button
                      id={product._id}
                      key={color}
                      className={"color btn btn-outline-dark me-2 " + isActive}
                      style={{ background: color }}
                      value={color}
                      onClick={(e) => changeColor(e.target, product)}
                    ></button>
                  );
                })}
              </div>
              <button className="btn btn-outline-dark my-2 px-4 py-2">
                Essayer
              </button>
              <button
                className="btn btn-dark my-2 ms-2 px-3 py-2"
